    ...
    <x-app-layout>
        <x-slot name="header">
            <div class="flex justify-between items-center">
                {{-- Título cambiado de font-black a font-semibold --}}
                <h2 class="font-semibold text-xl text-custom-primary leading-tight tracking-tight">
                    {{ __('Gestión de Personal') }}
                </h2>

                <div class="flex items-center space-x-3">
                    {{-- Botón Importar: Ahora usa bg-custom-secondary --}}
                    <button 
                        type="button"
                        @click="openModal = true" 
                        class="bg-custom-secondary hover:opacity-90 text-white px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all flex items-center shadow-sm"
                    >
                    ...
                    {{-- Botón Nuevo Ingreso: Ahora usa bg-custom-secondary --}}
                    <a href="{{ route('employees.create') }}" class="bg-custom-secondary hover:opacity-90 text-white font-black py-2 px-4 rounded-lg text-[10px] uppercase tracking-widest shadow-sm flex items-center transition-all">
                        <svg class="w-3.5 h-3.5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {{-- Ajustado a py-12 para igualar la distancia superior de la primera imagen --}}
        <div class="py-12">
                    ...
                    <table class="w-full text-left border-collapse">
                       <thead>
                            <tr class="bg-zinc-50/50 dark:bg-zinc-800/30 border-b border-zinc-100 dark:border-zinc-800">
                                <th class="p-5 text-xs font-black uppercase tracking-widest text-custom-primary">
                                    {{ __('Personal') }}
                                </th>
                                <th class="p-5 text-xs font-black uppercase tracking-widest text-custom-primary text-center">
                                    {{ __('Identificación') }}
                                </th>
                                <th class="p-5 text-xs font-black uppercase tracking-widest text-custom-primary text-center">
                                    {{ __('Contacto') }}
                                </th>
                                <th class="p-5 text-xs font-black uppercase tracking-widest text-custom-primary text-right">
                                    {{ __('Acciones') }}
                                </th>
                            </tr>
                        </thead>
                                ...
                                <td class="p-5 text-right">
                                    <div class="flex items-center justify-end space-x-1">
                                        ...
                                        <form action="{{ route('employees.destroy', $employee) }}" method="POST" class="inline-flex m-0 p-0">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="inline-flex items-center justify-center p-2 text-zinc-400 hover:text-red-500 transition-colors focus:outline-none" onclick="return confirm('¿Eliminar registro?')" title="Eliminar">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div x-show="openModal" x-cloak class="fixed inset-0 z-[999] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 w-full max-w-md p-8 shadow-2xl"
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button @click="openModal = false" class="text-zinc-400 hover:text-zinc-600 dark:hover:text-white transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
